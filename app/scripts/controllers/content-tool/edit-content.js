...
angular.module('statsdsuApp')
  .controller('EditContentCtrl',
    function ($scope, $location, $window, $compile, $routeParams, $firebaseObject, $log,
              SECArray, dragulaService, $rootScope) {
      $rootScope.ads = false;
      //Get SEC object
      SECArray.reset()
      $scope.docId = $routeParams.id;
      $scope.docType = $routeParams.type;
      var view = $location.path().split('/')[1];
      if(view==="edit-content-research"){
        $scope.cnt = SECArray.readResearchFromFirebase($routeParams.id)
        $scope.cnt.$loaded(function(){
          $scope.contestFromSECs = SECArray.getCnt();
          $log.info("editContentCtrl: ",$scope.contestFromSECs)
          SECArray.addUID();
          bindHtml($scope.contestFromSECs)
        })
      }
      else{
        $scope.cnt = SECArray.readContentFromFirebase($routeParams.type, $routeParams.id)
        $scope.cnt.$loaded(function(){
          $scope.contestFromSECs = SECArray.getCnt();
          $log.info("editContentCtrl: ",$scope.contestFromSECs)
          SECArray.addUID();
      ...
      function bindHtml(cnt) {
        ...
        cnt.forEach(function(val, index){
          if(val.type === 'editor-text'){
            ...
            container.attr('dragular','test-bag');
            container.attr('dragula-model','val');
            var courseCnt = angular.element(document.createElement('se-steps-edit'));
            courseCnt.attr('cnt', 'cnt.cnt['+index+']');
            courseCnt.attr('mode', 'edit');
            courseCnt.attr('target', index);
            container.append(courseCnt)
